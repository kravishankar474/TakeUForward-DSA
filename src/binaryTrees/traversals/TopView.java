...
3. Perform the following steps while the queue is not empty:
		1. Dequeue the node at the front of the queue and retrieve its vertical position. If this vertical position is not yet present in the map, 
...
public class TopView {
	...
	public List<Integer> topView(TreeNode root) {
		...
		while (!q.isEmpty()) {
			...
			// If the vertical position is not already in the map, add the node's data to
			// the map
	...
	public static void main(String[] args) {
		...
		List result = TopView.topView(root);
